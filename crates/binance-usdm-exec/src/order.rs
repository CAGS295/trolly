//! Request builder for Binance USDM REST `POST /fapi/v1/order`.
...
/// Position side for hedge mode (`LONG` / `SHORT`) or one-way mode (`BOTH`).
...
impl std::fmt::Display for OrderBuildError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::InvalidPositionSide(v) => write!(f, "invalid position side: {v}"),
...
impl NewOrderRequest {
    ...
    pub fn to_params(&self) -> Result<BTreeMap<String, String>, OrderBuildError> {
        ...
        params.insert("positionSide".into(), self.position_side.as_str().into());
...
mod tests {
    ...
    #[test]
    fn limit_order_params_include_price_position_side_and_time_in_force() {
        ...
        assert_eq!(params.get("positionSide"), Some(&"LONG".into()));
    ...
    fn market_order_params_omit_price_and_tif() {
        let order =
            NewOrderRequest::market("ETHUSDT", OrderSide::Sell, "1.0").timestamp_ms(1_700_000_000_000);
    ...
    #[test]
    fn converts_strategy_order_request_with_default_position_side() {
