//! Mock HTTP integration tests for signed USDM order placement.

use std::sync::{Arc, Mutex};

use binance_usdm_exec::{
    build_order, ApiCredentials, NewOrderRequest, OrderSide, OrderBuildError, PositionSide,
    TimeInForce, UsdmExecEgress, UsdmOrderClient,
};
...
async fn place_order_posts_signed_limit_request() {
    ...
    let client = UsdmOrderClient::with_base_url(base_url, test_credentials());
    ...
    assert!(request.contains("POST /fapi/v1/order?"));
    ...
    assert!(request.contains("positionSide=LONG"));
...
async fn place_order_posts_signed_market_request() {
    ...
    let client = UsdmOrderClient::with_base_url(base_url, test_credentials());
    let order = NewOrderRequest::market("ETHUSDT", OrderSide::Sell, "1.0")
    ...
    assert!(request.contains("positionSide=BOTH"));
    assert!(!request.contains("timeInForce="));
    assert!(!request.contains("price="));
}

#[test]
fn build_order_market_and_limit_with_position_side() {
    let market = build_order("BTCUSDT", "buy", "1", None, None, None).unwrap();
    assert_eq!(market.order_type, binance_usdm_exec::OrderType::Market);
    assert_eq!(market.position_side, PositionSide::Both);

    let limit = build_order("BTCUSDT", "SELL", "1", Some("99"), Some("SHORT"), Some("IOC")).unwrap();
    assert_eq!(limit.time_in_force, Some(TimeInForce::Ioc));
    assert_eq!(limit.position_side, PositionSide::Short);
}
...
fn egress_dispatches_strategy_order_request() {
    ...
    let client = UsdmOrderClient::with_base_url(base_url, test_credentials());
    let mut egress = UsdmExecEgress::new(client);
...
fn build_order_rejects_invalid_side() {
    let err = build_order("BTCUSDT", "HODL", "1", None, None, None).unwrap_err();
    assert!(matches!(err, OrderBuildError::InvalidSide(_)));
}

#[test]
fn api_error_surfaces_typed_result() {
    let rt = tokio::runtime::Runtime::new().unwrap();
    rt.block_on(async {
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let addr = listener.local_addr().unwrap();
        ...
        let client = UsdmOrderClient::with_base_url(format!("http://{addr}"), test_credentials());
        let order = NewOrderRequest::market("BTCUSDT", OrderSide::Buy, "1")
            .timestamp_ms(1_700_000_000_000);
        ...
        match err {
            binance_usdm_exec::OrderError::Api { status, body } => {
