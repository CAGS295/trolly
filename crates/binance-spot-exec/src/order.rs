...
//!
//! Fills and rejects are reconciled via the existing user-data `executionReport`
...
/// REST API host for spot order placement.
...
/// Binance REST acknowledgement for a placed order.
...
/// Signed spot order client.
#[derive(Clone, Debug)]
pub struct SpotOrderClient<T: OrderTransport> {
    pub credentials: ApiCredentials,
    pub base_url: String,
    transport: T,
}

impl<T: OrderTransport> SpotOrderClient<T> {
    ...
    pub async fn place_order(
        ...
        let url = format!("{}/api/v3/order", self.base_url.trim_end_matches('/'));
...
pub fn build_order_params(request: &PlaceOrderRequest) -> Result<BTreeMap<String, String>, OrderBuilderError> {
...
pub fn parse_place_order_response(response: HttpResponse) -> Result<PlaceOrderResponse, PlaceOrderError> {
...
mod tests {
    ...
    fn signed_payload_matches_hmac_fixture() {
        ...
        assert_eq!(
            payload,
            "price=50000.00&quantity=0.01&side=BUY&symbol=BTCUSDT&timeInForce=GTC&timestamp=1747385641636&type=LIMIT"
        );
    ...
    fn parse_ack_fixture() {
        ...
        assert_eq!(ack.order_id, 28_456_392);
    ...
    async fn place_order_posts_signed_form_to_rest_endpoint() {
        ...
        assert_eq!(captured[0].url, "https://api.binance.com/api/v3/order");
