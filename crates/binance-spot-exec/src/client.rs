...
pub const DEFAULT_REST_API_URL: &str = "https://api.binance.com";

/// Transport hook for `POST /api/v3/order` (mocked in tests; minimal HTTPS in production).
...
/// Signed REST client for Binance spot order placement.
pub struct SpotOrderClient<T: OrderHttpTransport = NativeTlsOrderTransport> {
    credentials: ApiCredentials,
    base_url: String,
    transport: T,
}

impl SpotOrderClient<NativeTlsOrderTransport> {
...
impl<T: OrderHttpTransport> SpotOrderClient<T> {
    ...
    /// Place an order via signed REST. Fills/rejects reconcile via user-data `executionReport`.
...
impl fmt::Display for OrderError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UnsupportedEgress(kind) => write!(f, "spot egress cannot dispatch {kind}"),
        }
    }
}
...
mod tests {
    ...
    fn place_order_posts_signed_form_to_rest_endpoint() {
        ...
        assert_eq!(url, "https://api.binance.com/api/v3/order");
