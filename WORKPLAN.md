...
- worker (2026-06-23): `ppo/` module with `PpoConfig`, `WolfPpoConfig`, `ActorCritic`, `PpoTrainer`, `WolfPpoTrainer`; clipped surrogate + WoLF LR switching; README WoLF-PPO section; `cargo test -p trolly-gym` and `--features torch` pass.
...
- worker (2026-06-23): `games/` module with matrix games, NES policies, self-play loop, NES distance metric; smoke + `#[ignore]` benchmark; README matrix-game section.
